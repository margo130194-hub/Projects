...
//  Pet App
//
//  Created by Margarita Matsonko on 29/06/2026.
//

import UIKit

final class CharacteristicsView : UIView {
    
    private var items: [Characteristics] = []
    ...
    func setupSubviews(){
        nameLabel.text = "Key Characteristics"
    ...
    func configure(with items: [Characteristics]) {
        self.items = items
        collectionView.reloadData()
    }
}

extension CharacteristicsView: UICollectionViewDataSource{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        items.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CharacteristicsCell.identifier, for: indexPath) as? CharacteristicsCell else {
            return UICollectionViewCell()
        }
        let characteristic = items[indexPath.item]
